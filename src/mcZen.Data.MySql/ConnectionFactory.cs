...
namespace mcZen.Data.MySql
{
	...
	public class ConnectionFactory
	{
		...
		public ScalarCommand<T> Register<T>(T defaultValue, string query, params MySqlConnector.MySqlParameter[] sqlParameters)
		{
		...
		/// <returns>Index of the command</returns>
		public int Register(string query, params MySqlConnector.MySqlParameter[] sqlParameters)
		{
			_Commands.Add(new mcZen.Data.MySql.Command(query, sqlParameters));
			return _Commands.Count - 1;
		}
		...
		public List<int> Execute()
		{
			...
			using (MySqlConnector.MySqlConnection conn = new MySqlConnector.MySqlConnection(_ConnectionString))
			{
				conn.Open();
				MySqlConnector.MySqlTransaction trans = conn.BeginTransaction();
				try
				{
		...
		public async Task<IEnumerable<int>> ExecuteAsync(System.Threading.CancellationToken cancellationToken = default)
		{
			...
			using (MySqlConnector.MySqlConnection conn = new MySqlConnector.MySqlConnection(_ConnectionString))
			{
				...
				var trans = (MySqlConnector.MySqlTransaction)await conn.BeginTransactionAsync(cancellationToken);
				try
				{
					int initialized = _Commands.Count;
					for (int i = 0; i < initialized; i++)
		...
		public static int Execute(string query, int timeout, string connectionString, params MySqlConnector.MySqlParameter[] parameters)
		{
			return Execute(new mcZen.Data.MySql.Command(query, System.Data.CommandType.Text, timeout, parameters), connectionString);
		}
